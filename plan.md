...
**M8.5 — Android Hexagon NPU.** ✅ Built. Uses `ghcr.io/snapdragon-toolchain/arm64-android:v0.3` (~8 GB image) which bundles Android NDK + Hexagon SDK 6.4 + OpenCL SDK. Configures with `-DGGML_HEXAGON=ON -DGGML_OPENCL=ON`. Output adds `libggml-opencl.so` + `libggml-hexagon.so` (host-side ARM64) plus six DSP-side libs `libggml-htp-v{68,69,73,75,79,81}.so` (Hexagon ELF) covering Snapdragon 865 → 8 Elite. Total AAR is ~3.7 MB stripped. Runtime selects the matching HTP variant per device; falls back to CPU on non-Snapdragon. Build script: `tool/build_android_hexagon_aar.sh`. Build flag tweaks needed under Rosetta 2 (Apple Silicon): `-flto` dropped + `-j 2` to keep cpp-httplib compile from OOM-killing. Native amd64 CI runners don't need this. **Not yet validated on a physical Snapdragon device** — ship as `0.9.x` once tested.
